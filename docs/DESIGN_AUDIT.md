...
| 54 | 🟡 | `SelectContent` : `className="bg-slate-800 border-white/20 text-white"` hardcodé → popup sombre en mode clair. Même pattern que le dropdown-menu kebab. | Retirer toutes les couleurs, laisser `bg-popover text-popover-foreground` du shadcn. |
...
| 56 | 🟢 | `Objectives` : la checkbox « completed » est **à droite** du champ texte objectif + points. Flux de lecture : texte → points → état. Temporel inversé : on remplit l'objectif, on le score, puis on revient cocher qu'il est fait. | Réordonner : checkbox | texte objectif | points. Checkbox à gauche = « cocher puis détailler ». |
| 57 | 🟢 | `<Input type="number" min={0} max={999}>` dans les scores : `onChange` fait `parseInt(e.target.value) || 0`. Problème : si l'utilisateur efface le champ pour retaper, `""` devient `0` instantanément (chaîne vide → NaN → 0), l'utilisateur ne peut pas « vider » le champ. | Typer en `string` dans le state, parseInt uniquement à la soumission : `<Input value={scoreStr} onChange={(e) => setScoreStr(e.target.value)} />` + validation `parseInt >= 0`. |
| 58 | 🟢 | Les `<Label>` ne sont pas toujours liés aux `<Input>` par `htmlFor`. Lecteur d'écran n'annonce pas le label au focus sur le champ. | `<Label htmlFor="duration">Durée</Label><Input id="duration" />`. Pattern shadcn standard. |
...
| 63 | 🟡 | Search dans un wrapper `bg-slate-50 text-slate-900 p-4 rounded-xl border border-slate-200` en mode clair — c'est effectivement un wrapper, mais cette couleur `slate-50` contredit le fond de la page environnante (qui est `bg-white` ou gradient). Double fond. | Retirer le wrapper ou aligner sur `bg-background` / `bg-card`. |
| 64 | 🟢 | Spinner `<Circle className="animate-spin">` dans le bouton search et dans le loading state : aucun `aria-label` ni `role="status"`. NVDA n'annonce pas « recherche en cours ». | `<span role="status" aria-live="polite"><Circle aria-hidden="true" />Recherche en cours</span>`. |
| 65 | 🟢 | Footer `text-xs text-white/40 text-center` : « Données fournies par BoardGameGeek ». Contraste échec AA. | `text-white/60` min. Et en mode clair, `text-slate-400` devient `text-slate-500` minimum. |
