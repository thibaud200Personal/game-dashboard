...
- **🏷️ `ApiService.getImportLog()` — type de retour inexact** — La méthode déclare `{ bgg_catalog_imported_at, data_exported_at, data_imported_at }` mais l'endpoint `/api/v1/bgg/import-status` retourne `{ count, bgg_catalog_imported_at }`. Les champs `data_exported_at` et `data_imported_at` sont `undefined` en runtime sans erreur TypeScript. À corriger lors de la migration de `ApiService.ts` vers React Query : remplacer `getImportLog()` + `getBggCatalogStatus()` par un seul hook `useBggCatalogStatus()` avec le type `{ count: number; bgg_catalog_imported_at: string | null }`.
